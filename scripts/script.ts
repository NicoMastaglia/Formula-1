...
const standingBtn = document.getElementById(
  "standing-btn",
) as HTMLButtonElement;
const resultBtn = document.getElementById("result-btn") as HTMLButtonElement;
const buttonBg = document.getElementById("test") as HTMLDivElement;
...
async function loadResultPage() {
  ...
  const raceList = document.querySelector(".race-list") as HTMLDivElement;
...
async function printConstructorStanding(data: any) {
...
async function printDriverStanding(data: any) {
  const driverrStanding = document.getElementById(
    "driver-standing-data",
  ) as HTMLDivElement;
...
async function getRaceList() {
  ...
  const url = `https://api.jolpi.ca/ergast/f1/${year}/`;
...
async function printRaceList(data: any) {
  const raceContainer = document.querySelector(".race-list") as HTMLDivElement;
  if (typeof data != "string") {
...
async function getRaceResult(round: string | number) {
  const year = parseInt(
    (document.getElementById("year") as HTMLInputElement).value,
  );
  const url = `https://api.jolpi.ca/ergast/f1/${year}/${round}/results`;
...
async function printRaceResult(data: any) {
  const loadData = await data;
  console.log(loadData);
  const BOX = document.querySelector(".race-result") as HTMLDivElement;
  BOX.innerHTML = "";
  if (typeof loadData == "string") {
    ...
    const DBOX = document.querySelector(".race-standing") as HTMLDivElement;
